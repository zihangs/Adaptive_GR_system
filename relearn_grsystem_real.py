...
if __name__ == "__main__":
    ...
    option = sys.argv[1]
    domain = sys.argv[2]
    obs_percentage = sys.argv[3]

    ################################ parameters ###################################
    # domain = "sepsis_cases_4"
    # obs_percentage = 1.0
    ...
    test_dataset = "real_world_logs/%s/testing" % domain
    training_dataset = "real_world_logs/%s/training" % domain

    output_dir = "results"
    ###############################################################################
    output_stats = "real_world_logs/out%s-%s_%s.csv" % (option, domain, obs_percentage)
    gr_system = GRsystem(training_dataset, test_dataset)
